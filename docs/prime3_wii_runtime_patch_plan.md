...
`--ios-get-host-id-once` reuses the same dormant internal mode and extends that proof by exactly one additional operation after `SO_STARTUP`, then stops before any socket creation. After the successful startup callback, it calls the verified retail async ioctl wrapper at `0x80504FE0` with `r3 = ip_fd`, `r4 = 16`, `r5 = 0`, `r6 = 0`, `r7 = 0`, `r8 = 0`, `r9 = runtime_ios_callback`, and `r10 = project-owned persistent GET_HOST_ID context`. The request shape remains exactly `NULL, 0, NULL, 0`; there is no input buffer, no output buffer, and no hidden result storage. The runtime records the target, command, submitted fd, callback/context pointers, `r3..r10` pre-call snapshot, callback exit count, stale/duplicate callback counts, `service_started` before/after, `ip_fd` before/after, pending/phase transitions, raw callback `s32` bits, raw callback `u32` bits, host-ID bytes, and host-ID availability/ready flags into manifest-backed runtime state. The verified live Dolphin result is synchronous result `0`, one callback, matching generation `6`, preserved host-ID bits `0x1AD38AB6`, bytes `1A D3 8A B6`, dotted IPv4 `26.211.138.182`, retained `ip_fd = 11`, `service_started = true`, and terminal `HOST_ID_READY` (`19`) with no socket, bind, receive, send, CP3W, or mailbox activity. For this operation, callback result `0` means unavailable; every nonzero 32-bit pattern is preserved exactly rather than tested with signed `< 0` failure logic.
