...
def _make_devkit_tree(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for name in (
        ...
        devkitppc.joinpath("bin", name).write_text("", encoding="utf-8")
...
def test_resolve_prime3_runtime_toolchain_rejects_wrong_target(
